...
let
  sessionConfiguration = import ../sessions/${session}.nix { inherit pkgs inputs; };
  hostConfiguration = {
    ...
    system = {
      ...
      checks = {
        # Verify Nix build users validation, and macOS version.
...
in
lib.recursiveUpdate hostConfiguration sessionConfiguration
